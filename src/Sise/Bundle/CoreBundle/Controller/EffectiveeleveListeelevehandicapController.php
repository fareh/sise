...
class EffectiveeleveListeelevehandicapController extends Controller
{
    ...
    public function editAction(Request $request)
    {
        ...
        $pathUpdate = $this->generateUrl('effectiveelevelisteelevehandicap_update', array('codeetab' => $codeetab, 'codetypeetab' => $codetypeetab, 'annescol' => $annescol, 'coderece' => $coderece));
    ...
    public function updateAction(Request $request, $codeetab, $codetypeetab)
    {
        ...
        if ($codeetab && $codetypeetab) {
            ...
            for ($i = 0; $i < count($entities); $i++) {
                ...
                $item = $em->getRepository('SiseCoreBundle:EffectiveeleveListeelevehandicap')->findOneBy($items);
                $item->setNombelevresimasc($request->request->get('sise_bundle_corebundle_effectiveelevelisteelevehandicap_nombelevresimasc_' . $i));
                $item->setNombelevresifemi($request->request->get('sise_bundle_corebundle_effectiveelevelisteelevehandicap_nombelevresifemi_' . $i));
                $item->setNombtotaresielev($request->request->get('sise_bundle_corebundle_effectiveelevelisteelevehandicap_nombtotaresielev_' . $i));
                $item->setNombelevbourfemi($request->request->get('sise_bundle_corebundle_effectiveelevelisteelevehandicap_nombelevbourfemi_' . $i));
                $item->setNombtotabourelev($request->request->get('sise_bundle_corebundle_effectiveelevelisteelevehandicap_nombtotabourelev_' . $i));
                $em->persist($item);
