...
class EffectifeleveListeelevedeplacerversautreetablissementController extends Controller
{
    ...
    public function editAction(Request $request)
    {
        ...
        if ($codeetab && $codetypeetab) {
            ...
            if (count($entities) > 0) {
                ...
                $editForms[1] = $this->createEditForm(new EffectifeleveListeelevedeplacerversautreetablissement(), 1)->createView();
            }
        ...
        $nameclass = $em->getRepository('SiseCoreBundle:NomenclatureQuestionnaire')->findOneByNameclass('effectifeleve_listeelevedeplacerversautreetablissement');
        return $this->render('SiseCoreBundle:NomenclatureQuestionnaire:edit.effectifeleve_listeelevedeplacerversautreetablissement.html.twig', array(
            'entities' => @$entities,
            'editForms' => @$editForms,
    ...
    private function createEditForm(EffectifeleveListeelevedeplacerversautreetablissement $entity, $key)
    {
        $form = $this->createForm(new EffectifeleveListeelevedeplacerversautreetablissementType($key), $entity);
        return $form;
    }
    ...
    public function deleteItemAction(Request $request)
    {
        if ($request->isXmlHttpRequest()) // pour vérifier la présence d'une requete Ajax
        {
            ...
            if ($codeetab != '' and  $codetypeetab != '' and  $annescol != '' and  $coderece != '' and  $numeelev != ''  ) {
                $item = $em->getRepository('SiseCoreBundle:EffectifeleveListeelevedeplacerversautreetablissement')->findOneBy(array('codeetab' => $codeetab, 'codetypeetab' => $codetypeetab, 'annescol' => $annescol, 'coderece' => $coderece, 'numeelev' => $numeelev));
                if ($item) {
                    $em->remove($item);
                    $em->flush();
                }
    ...
    public  function itemAction(Request $request)
    {
        if ($request->isXmlHttpRequest()) // pour vérifier la présence d'une requete Ajax
        {
            ...
            if ($newcompteur != '') {
                $form = $this->createEditForm(new EffectifeleveListeelevedeplacerversautreetablissement(), $newcompteur)->createView();
                return $this->render('SiseCoreBundle:Default:prototype_listeelevedeplacerversautreetablissement.html.twig', array(
                    'form' => @$form,
                ));
        ...
        if ($codeetab && $codetypeetab && $request->isMethod('POST')) {
            foreach ($request->request as $numeelev => $parameters) {
                ...
                if($item){
                    ...
                    $item = new EffectifeleveListeelevedeplacerversautreetablissement($codeetab, $codetypeetab, $annescol, $coderece,$numeelev);
    ...
    public function listAction(Request $request)
    {
        ...
        $nameclass = $em->getRepository('SiseCoreBundle:NomenclatureQuestionnaire')->findOneByNameclass('effectifeleve_listeelevedeplacerversautreetablissement');
        return $this->render('SiseCoreBundle:NomenclatureQuestionnaire:list.effectifeleve_listeelevedeplacerversautreetablissement.html.twig', array(
            'entities' => @$entities,
            'search' => $search->createView(),
            'pathfilter' => $url,
            'nameclass' => $nameclass
